...
type PTWPreviewData = {
  ptw: {
    ...
    primary_feeders: Record<string, { grid_id: number; grid_code: string; feeders: { primary: Array<{ id: number; name: string; code: string; is_on: boolean }> } }>;
    ...
    logs: { id: number; action: string; role: string; notes: string | null; created_at: string; actor_name: string }[];
...
const badgeColor = (status: string) => {
  const map: Record<string, string> = {
    ...
    GRID_RESTORED_AND_CLOSED: "bg-blue-100 text-blue-800", // Custom for new status
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <div className="sticky top-0 border-b bg-white/80 backdrop-blur-sm z-10">
        <div className="mx-auto max-w-6xl px-6 py-4 flex justify-between items-center">
          <h1 className="text-xl font-semibold text-slate-800">PTW Preview /
      ...
      <div className="mx-auto max-w-6xl px-6 py-6 space-y-6">
        ...
        <div className="border rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="text-sm font-semibold mb-3">Summary /
             <span className="font-urdu">
             خلاصہ 
             </span>
             </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p><b>Feeder:</b> {ptw.feeder_incharge_name}</p>
            </div>
            <div>
             <p><b>Due Time:</b> {ptw.due_time ? new Date(ptw.due_time).toLocaleString() : '—'}</p>
        ...
        {/* Work Details */}
        <div className="border rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="text-sm font-semibold  mb-3">Work Details /
             <span className="font-urdu">
        ...
        {/* Team Members */}
        <div className="border rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="text-sm font-semibold mb-3">Team Members / 
              <span className="font-urdu">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {ptw.team_members.map((m) => (
                <div key={m.id} className="flex items-center gap-3 border rounded-lg p-2">
                  <img src={m.avatar_url} alt={m.name} className="w-10 h-10 rounded-full border object-cover" />
                  <div>
        ...
        {/* Evidence Photos */}
        <div className="border rounded-2xl bg-white p-6 shadow-sm">
          <h2 className="text-sm font-semibold mb-3">Evidence Photos / شواہد</h2>
          {ptw.evidences.length ? (
            <div className="flex flex-wrap gap-4">
              {ptw.evidences.map((e) => (
                <div key={e.id} className="w-32">
                  <img src={storageUrl(e.file_path)} alt={e.type} className="w-32 h-28 object-cover border rounded-lg" />
        ...
        {(["LINE_TYPE", "HAZARDS"] as const).map((key) => (
          <div key={key} className="border rounded-2xl bg-white p-6 shadow-sm space-y-3">
            <h2 className="text-sm font-semibold capitalize">{key.replaceAll("_", " ")} Checklist</h2>
              ...
              {checklists[key]?.map((item) => (
                <div key={item.id} className="flex items-start justify-between border rounded-md p-3">
                  <div>
                    ...
                    <p className="text-xs font-urdu text-slate-500" dir="rtl">
                      {item.label_ur}
                    </p>
                  </div>
                  {chip(item.value)}
