...
// ---- Dummy rows ----
type Status = "Pending" | "Ready" | "Started";
type Row = {
  id: string;             // PTW No
  title: string;
  division: string;
  ...
  crewCount: number;
  ppeOk: boolean;         // quick flag
  gpsOk: boolean;         // quick flag
  status: Status;
};

const ROWS: Row[] = [
  { id: "PTW-1601", title: "Feeder F-101 Maintenance", division: "Gulberg Division", date: "2025-09-23", crewCount: 4, ppeOk: false, gpsOk: false, status: "Pending" },
  { id: "PTW-1602", title: "Transformer T-55 Inspection", division: "DHA Division",    date: "2025-09-24", crewCount: 3, ppeOk: true,  gpsOk: true,  status: "Ready"   },
  { id: "PTW-1603", title: "Line L-22 Hotspot Repair",   division: "Satellite Town",   date: "2025-09-22", crewCount: 5, ppeOk: true,  gpsOk: true,  status: "Started" },
];

const ALL_DIVISIONS = Array.from(new Set(ROWS.map(r => r.division))).sort();
const ALL_STATUS: Status[] = ["Pending", "Ready", "Started"];
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      ...
      <div className="col-span-12 flex items-center h-10 intro-y mt-4">
        <h2 className="mr-5 text-lg font-medium truncate">Pre-Start — Crew & PPE</h2>
      ...
      <div className="col-span-12 intro-y">
        <div className="box p-5 rounded-2xl shadow-sm border border-slate-200/60 dark:border-darkmode-300 bg-gradient-to-b from-white to-slate-50 dark:from-darkmode-700 dark:to-darkmode-600">
          <div className="grid grid-cols-12 gap-4">
            ...
            <div className="col-span-12 sm:col-span-6 lg:col-span-3">
              <FormInput type="date" value={date} onChange={e => { setPage(1); setDate(e.target.value); }} />
            </div>
          ...
          <div className="mt-3">
            <Button
              ...
              onClick={() => { setQ(""); setDivision("all"); setStatus("all"); setDate(""); setPage(1); }}
            >
              Reset
            </Button>
          </div>
        ...
        <div className="box p-0 rounded-2xl shadow-sm border border-slate-200/60 dark:border-darkmode-300 overflow-hidden">
          <div className="overflow-auto">
            <Table className="table-auto w-full">
              <Table.Thead>
                <Table.Tr>
                  {["PTW No", "Title", "Division", "Crew", "PPE", "GPS", "Status", "Actions"].map(h => (
                    <Table.Th
                      key={h}
                      className={
              ...
              <Table.Tbody>
                {paged.map(r => (
                  <Table.Tr key={r.id} className={clsx("group transition-all duration-200", density === "compact" ? "text-[13px]" : "text-sm")}>
                    ...
                    <Table.Td className={clsx("px-5", cellY)}>{r.crewCount}</Table.Td>
                    <Table.Td className={clsx("px-5", cellY)}>
                      <span className={clsx("inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full",
                        r.ppeOk ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700")}>
                        <span className={clsx("h-1.5 w-1.5 rounded-full", r.ppeOk ? "bg-emerald-500" : "bg-amber-500")} />
                        {r.ppeOk ? "OK" : "Missing"}
                      </span>
                    </Table.Td>
                    <Table.Td className={clsx("px-5", cellY)}>
                      <span className={clsx("inline-flex items-center gap-1 text-[11px] px-2 py-1 rounded-full",
                        r.gpsOk ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700")}>
                        <span className={clsx("h-1.5 w-1.5 rounded-full", r.gpsOk ? "bg-emerald-500" : "bg-amber-500")} />
                        {r.gpsOk ? "OK" : "Missing"}
                      </span>
                    </Table.Td>
                    <Table.Td className={clsx("px-5", cellY)}>
                    ...
                    <Table.Td className={clsx("px-5", cellY, "text-center")}>
                      <Button
                        ...
                        onClick={() => navigate(`/pre-start/${r.id}`)}
                      >
                        <Lucide icon="PlayCircle" className="w-4 h-4 mr-1" /> Pre-Start
                      </Button>
                    </Table.Td>
                  </Table.Tr>
                ))}
